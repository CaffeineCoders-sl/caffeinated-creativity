...
const Services = ({ initialCategory = 'all', showFilter = true }: ServicesProps) => {
  ...
  return (
    <div className="bg-white" ref={rootRef}>
      ...
      {(activeCategory === 'ai' || activeCategory === 'all') && (
        <section className="py-20 bg-gradient-to-b from-black to-white/80">
          <div className="container mx-auto px-4">
            ...
            >
              <h2 className="text-4xl font-bold text-white mb-4">AI & Machine Learning Solutions</h2>
              <p className="text-xl text-gray-300 max-w-3xl mx-auto">
                Harness the power of artificial intelligence to gain insights, automate processes, and create
              ...
              <div className="flex flex-col md:flex-row items-center gap-8">
                ...
                >
                  <div className="bg-black p-1 rounded-lg shadow-xl">
                    <div className="bg-gradient-to-tr from-black to-gray-800 rounded-lg overflow-hidden">
                      {aiSpotlights.map((spotlight, index) => (
                        <AnimatePresence key={index} initial={false} mode="wait">
                          {spotlightService === index && (
                            <motion.div
                              ...
                              className="aspect-w-16 aspect-h-9 bg-black/40 p-8 flex items-center justify-center"
                            >
                              <div className="text-center">
                                <h3 className="text-3xl font-bold text-white mb-4">{spotlight.title}</h3>
                                <p className="text-gray-300 mb-6">{spotlight.description}</p>
                                {/* Added technology tags for more detail */}
                                <div className="flex flex-wrap justify-center gap-2">
                                  {spotlight.technologies.map((tech, i) => (
                  ...
                  {/* Added interactive dots navigator */}
                  <div className="flex justify-center mt-4 space-x-2">
                    {aiSpotlights.map((_, index) => (
                      <button
                        key={index}
                        onClick={() => setSpotlightService(index)}
                ...
                >
                  {aiSpotlights.map((spotlight, index) => (
                    <motion.div 
                      ...
                      onClick={() => setSpotlightService(index)}
                      whileHover={{ x: spotlightService === index ? 0 : 5 }}
                    >
                      <div className="flex items-center justify-between">
                        <h4 className={`font-bold ${spotlightService === index ? 'text-white' : ''}`}>
                          {spotlight.title}
                        </h4>
                        {spotlightService === index && (
                          <motion.div 
                  ...
                  <Link 
                    to="/contact" 
                    className="inline-block px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-700 text-white rounded-lg hover:shadow-lg transition-all mt-4 flex items-center"
                  >
